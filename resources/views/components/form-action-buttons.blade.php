<div class="pt-4 pb-1 mt-5 border-top">
    <div class="d-grid gap-3 d-sm-flex justify-content-md-start pt-1">
        @if (request()->routeIs($slot . '.show'))
            ...
            <a href="{{ route($slot . '.index') }}" class="btn btn-secondary px-4">
        ...
        @else
            {{-- button simpan data --}}
            <button type="submit" class="btn btn-secondary px-4">Simpan</button>
            {{-- button kembali ke halaman index --}}
            <a href="{{ route($slot . '.index') }}" class="btn btn-default px-4">Batal</a>
        @endif
    </div>
</div>