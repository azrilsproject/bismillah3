<x-app-layout>
    <x-slot:page>Laporan Barang Keluar</x-slot:page>

    {{-- Page Title --}}
    <x-page-title>Laporan Barang Keluar</x-page-title>

    <div class="card">
        <div class="card-body">
            {{-- judul form --}}
            <x-form-title>
                <i class="ti ti-filter fs-5 me-2"></i> Filter Data Barang Keluar
            </x-form-title>

            {{-- form filter data --}}
            <form action="{{ route('laporan-barang-keluar.filter') }}" method="GET">
                <div class="row">
                    <div class="col-lg-3">
                ...
                {{-- action buttons --}}
                <x-form-action-buttons>laporan-barang-keluar</x-form-action-buttons>
            </form>
        </div>
    </div>
                ...
                <div class="d-sm-flex justify-content-between align-items-center mb-4">
                    <div class="d-flex align-items-center">
                        ...
                        <h6 class="report-title mb-0">
                            ...
                            Laporan Data Barang Keluar Tanggal {{ Carbon\Carbon::parse(request('tgl_awal'))->translatedFormat('j F Y') }} s.d. {{ Carbon\Carbon::parse(request('tgl_akhir'))->translatedFormat('j F Y') }}
                        </h6>
                    </div>

                    <div class="d-grid gap-2 mb-3 mb-sm-0">
                        {{-- button cetak laporan (export PDF) --}}
                        <a href="{{ route('laporan-barang-keluar.print', [request('tgl_awal'), request('tgl_akhir')]) }}" target="_blank" class="btn btn-warning px-4">
                ...
                <div class="table-responsive border rounded mb-2">
                    <table class="table align-middle text-nowrap mb-0">
                        ...
                        @forelse ($barangKeluar as $data)
                            ...
                            <tr>
                                <td width="30">{{ $no++ }}</td>
                                <td width="80">{{ Carbon\Carbon::parse($data->tanggal)->translatedFormat('j F Y') }}</td>
                                <td width="80">{{ $data->barang_id }}</td>
                                <td width="200">{{ $data->nama_barang }}</td>        
                                <td width="100">{{ $data->sn }}</td> 
                                <td width="100">{{ $data->jumlah_keluar }}</td>
